...
module.exports = function (source) {
  ...
  // For ort bundle files: normalize the webpackIgnore comment to the spaced
  // format so webpack's parser is guaranteed to recognise it, even if the
  // compact /*webpackIgnore:true*/ form gets stripped by a transform step.
  if (this.resourcePath && (
    this.resourcePath.includes('ort.bundle') ||
    this.resourcePath.includes('ort.wasm.bundle') ||
    this.resourcePath.includes('ort-wasm-simd-threaded')
  )) {
    result = result.replace(
      /\/\*webpackIgnore:true\*\/ \/\*@vite-ignore\*\//g,
      '/* webpackIgnore: true */'
    );
  }
